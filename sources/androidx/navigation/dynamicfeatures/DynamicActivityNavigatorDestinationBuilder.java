...
public final class DynamicActivityNavigatorDestinationBuilder extends NavDestinationBuilder<ActivityNavigator.Destination> {
    ...
    public DynamicActivityNavigatorDestinationBuilder(DynamicActivityNavigator activityNavigator2, int id) {
        super(activityNavigator2, id);
        Intrinsics.checkParameterIsNotNull(activityNavigator2, "activityNavigator");
    ...
    public DynamicActivityNavigator.Destination build() {
        ...
        if (build != null) {
            ...
            DynamicActivityNavigator.Destination destination2 = destination;
            String it = this.activityClassName;
            if (it != null) {
                String str = this.targetPackage;
                if (str == null) {
                    str = this.activityNavigator.getPackageName();
                } else if (str == null) {
                    Intrinsics.throwNpe();
                }
                destination2.setComponentName(new ComponentName(str, it));
            }
            destination2.setTargetPackage(this.targetPackage);
            destination2.setModuleName(this.moduleName);
            destination2.setAction(this.action);
            destination2.setData(this.data);
            destination2.setDataPattern(this.dataPattern);
            return destination;
        }
