...
public final class DynamicNavGraphBuilder extends NavGraphBuilder {
    ...
    public DynamicNavGraphBuilder(NavigatorProvider provider, int id, int startDestination2) {
        super(provider, id, startDestination2);
        Intrinsics.checkParameterIsNotNull(provider, "provider");
        this.startDestination = startDestination2;
    }
    ...
    public NavGraph build() {
        ...
        NavGraph navGraph = build;
        if (navGraph instanceof DynamicGraphNavigator.DynamicNavGraph) {
            ((DynamicGraphNavigator.DynamicNavGraph) navGraph).setModuleName(this.moduleName);
            ((DynamicGraphNavigator.DynamicNavGraph) navGraph).setProgressDestination(this.progressDestination);
            if (this.progressDestination == 0) {
                Navigator navigator = getProvider().getNavigator(DynamicGraphNavigator.class);
                Intrinsics.checkExpressionValueIsNotNull(navigator, "getNavigator(clazz.java)");
                ((DynamicGraphNavigator) navigator).mo8136x65249a72().add(navGraph);
            }
        }
        return build;
    }
