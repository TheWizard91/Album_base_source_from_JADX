...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() {
        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f37x - o2.f37x;
            return cmpX == 0 ? o1.f38y - o2.f38y : cmpX;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f37x += range.oldListStart;
                snake.f38y += range.newListStart;
                Range left = arrayList.isEmpty() ? new Range() : (Range) arrayList.remove(arrayList.size() - 1);
                left.oldListStart = range.oldListStart;
                left.newListStart = range.newListStart;
                if (snake.reverse) {
                    left.oldListEnd = snake.f37x;
                    left.newListEnd = snake.f38y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f37x - 1;
                    left.newListEnd = snake.f38y;
                } else {
                    left.oldListEnd = snake.f37x;
                    left.newListEnd = snake.f38y - 1;
                }
                stack.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f37x + snake.size;
                    right.newListStart = snake.f38y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = snake.f37x + snake.size + 1;
                    right.newListStart = snake.f38y + snake.size;
                } else {
                    right.oldListStart = snake.f37x + snake.size;
                    right.newListStart = snake.f38y + snake.size + 1;
                }
                stack.add(right);
            } else {
                arrayList.add(range);
            ...
            for (int d = 0; d <= dLimit; d++) {
                ...
                while (k <= d) {
                    ...
                    if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || iArr[kOffset + k] < iArr2[kOffset + k]) {
                        ...
                        outSnake.f37x = iArr2[kOffset + k];
                        outSnake.f38y = outSnake.f37x - k;
                ...
                while (k2 <= d) {
                    ...
                    if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || iArr[kOffset + backwardK] < iArr2[kOffset + backwardK]) {
                        ...
                        outSnake2.f37x = iArr2[kOffset + backwardK];
                        outSnake2.f38y = outSnake2.f37x - backwardK;
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f37x != 0 || firstSnake.f38y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f37x + snake.size;
                int endY = snake.f38y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f37x + j;
                    int newItemPos = snake.f38y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f37x + snakeSize2;
                int endY2 = snake.f38y + snakeSize2;
                if (endX < posOld) {
                    endY = endY2;
                ...
                for (int i2 = snakeSize - 1; i2 >= 0; i2--) {
                    if ((this.mOldItemStatuses[snake.f37x + i2] & 31) == 2) {
                        batchingCallback.onChanged(snake.f37x + i2, 1, this.mCallback.getChangePayload(snake.f37x + i2, snake.f38y + i2));
                    }
                }
                posOld = snake.f37x;
                posNew = snake.f38y;
