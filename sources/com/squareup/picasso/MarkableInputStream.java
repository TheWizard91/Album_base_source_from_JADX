...
final class MarkableInputStream extends InputStream {
    ...
    private MarkableInputStream(InputStream in, int size, int limitIncrement2) {
        ...
        this.f185in = !in.markSupported() ? new BufferedInputStream(in, size) : in;
    ...
    private void setLimit(long limit2) {
        try {
            ...
            if (j >= j2 || j2 > this.limit) {
                this.reset = j2;
                this.f185in.mark((int) (limit2 - j2));
            } else {
                this.f185in.reset();
                this.f185in.mark((int) (limit2 - this.reset));
    ...
    private void skip(long current, long target) throws IOException {
        while (current < target) {
            long skipped = this.f185in.skip(target - current);
            if (skipped == 0) {
                if (read() != -1) {
                    skipped = 1;
                } else {
    ...
    public int read(byte[] buffer) throws IOException {
        ...
        int count = this.f185in.read(buffer);
        if (count != -1) {
    ...
    public int read(byte[] buffer, int offset2, int length) throws IOException {
        ...
        int count = this.f185in.read(buffer, offset2, length);
        if (count != -1) {
            this.offset += (long) count;
        }
        return count;
    ...
    public long skip(long byteCount) throws IOException {
        ...
        long skipped = this.f185in.skip(byteCount);
        this.offset += skipped;
        return skipped;
    }
