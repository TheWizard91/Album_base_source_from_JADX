...
public class CountingMemoryCache<K, V> implements MemoryCache<K, V>, MemoryTrimmable {
    ...
    static class Entry<K, V> {
        ...
        /* renamed from: of */
        static <K, V> Entry<K, V> m131of(K key2, CloseableReference<V> valueRef2, @Nullable EntryStateObserver<K> observer2) {
            return new Entry<>(key2, valueRef2, observer2);
        }
    }

    public CountingMemoryCache(ValueDescriptor<V> valueDescriptor, MemoryCache.CacheTrimStrategy cacheTrimStrategy, Supplier<MemoryCacheParams> memoryCacheParamsSupplier) {
        ...
        this.mMemoryCacheParams = memoryCacheParamsSupplier.get();
    ...
    public CloseableReference<V> cache(K key, CloseableReference<V> valueRef, EntryStateObserver<K> observer) {
        ...
        synchronized (this) {
            ...
            if (canCacheNewValue(valueRef.get())) {
                Entry<K, V> newEntry = Entry.m131of(key, valueRef, observer);
    ...
    private synchronized boolean canCacheNewValue(V value) {
        ...
        z = true;
        if (newValueSize > this.mMemoryCacheParams.maxCacheEntrySize || getInUseCount() > this.mMemoryCacheParams.maxCacheEntries - 1 || getInUseSizeInBytes() > this.mMemoryCacheParams.maxCacheSize - newValueSize) {
            z = false;
        }
        return z;
    }
    ...
    private synchronized CloseableReference<V> newClientReference(final Entry<K, V> entry) {
        ...
        return CloseableReference.m126of(entry.valueRef.get(), new ResourceReleaser<V>() {
