...
public class JpegTranscoderUtils {
    ...
    public static final ImmutableList<Integer> INVERTED_EXIF_ORIENTATIONS = ImmutableList.m30of(2, 7, 4, 5);
    ...
    public static int getForceRotatedInvertedExifOrientation(RotationOptions rotationOptions, EncodedImage encodedImage) {
        ...
        int index = immutableList.indexOf(Integer.valueOf(exifOrientation));
        if (index >= 0) {
            int forcedAngle = 0;
            if (!rotationOptions.useImageMetadata()) {
                forcedAngle = rotationOptions.getForcedAngle();
            }
            return ((Integer) immutableList.get((index + (forcedAngle / 90)) % immutableList.size())).intValue();
        }
