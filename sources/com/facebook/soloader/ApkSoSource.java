    ...
    protected class ApkUnpacker extends ExtractFromZipSoSource.ZipUnpacker {
        ...
        public boolean shouldExtract(ZipEntry ze, String soName) {
            ...
            if (soName.equals(ApkSoSource.this.mCorruptedLib)) {
                ...
                msg = String.format("allowing consideration of corrupted lib %s", new Object[]{soName});
                result = true;
                ...
                if (!sysLibFile.isFile()) {
                    msg = String.format("allowing considering of %s: %s not in system lib dir", new Object[]{zipPath, soName});
                    result = true;
                } else {
                    long sysLibLength = sysLibFile.length();
                    long apkLibLength = ze.getSize();
                    if (sysLibLength != apkLibLength) {
                        msg = String.format("allowing consideration of %s: sysdir file length is %s, but the file is %s bytes long in the APK", new Object[]{sysLibFile, Long.valueOf(sysLibLength), Long.valueOf(apkLibLength)});
                        result = true;
                    } else {
