...
public class EmailLinkCatcherActivity extends InvisibleActivityBase {
    ...
    private void initHandler() {
        EmailLinkSignInHandler emailLinkSignInHandler = (EmailLinkSignInHandler) ViewModelProviders.m17of((FragmentActivity) this).get(EmailLinkSignInHandler.class);
    ...
    public AlertDialog buildAlertDialog(final int errorCode) {
        ...
        if (errorCode == 11) {
            titleText = getString(C2354R.string.fui_email_link_different_anonymous_user_header);
            messageText = getString(C2354R.string.fui_email_link_different_anonymous_user_message);
        } else if (errorCode == 7) {
            titleText = getString(C2354R.string.fui_email_link_invalid_link_header);
            messageText = getString(C2354R.string.fui_email_link_invalid_link_message);
        } else {
            titleText = getString(C2354R.string.fui_email_link_wrong_device_header);
            messageText = getString(C2354R.string.fui_email_link_wrong_device_message);
        }
        return alertDialog.setTitle(titleText).setMessage(messageText).setPositiveButton(C2354R.string.fui_email_link_dismiss_button, new DialogInterface.OnClickListener() {
