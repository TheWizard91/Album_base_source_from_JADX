...
public class EmailLinkPersistenceManager {
    ...
    public SessionRecord retrieveSessionRecord(Context context) {
        ...
        if (!(provider == null || (idpToken == null && this.mCredentialForLinking == null))) {
            sessionRecord.setIdpResponseForLinking(new IdpResponse.Builder(new User.Builder(provider, email).build()).setPendingCredential(this.mCredentialForLinking).setToken(idpToken).setSecret(idpSecret).setNewUser(false).build());
        }
