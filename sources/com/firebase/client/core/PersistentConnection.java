...
public class PersistentConnection implements Connection.Delegate {
    ...
    private static class AuthCredential {
        ...
        static {
            Class<PersistentConnection> cls = PersistentConnection.class;
        }
        ...
        public void replay(Firebase.AuthListener listener) {
            Object obj = this.authData;
            if (obj != null) {
                listener.onAuthSuccess(obj);
                return;
            }
    ...
    public void onDisconnect(Connection.DisconnectReason reason) {
        ...
        if (!this.shouldReconnect) {
            ...
            if (reason == Connection.DisconnectReason.SERVER_RESET) {
                ...
                timeSinceLastConnectAttempt = (long) this.random.nextInt((int) Math.max(1, this.reconnectDelay - (System.currentTimeMillis() - this.lastConnectionAttemptTime)));
            }
            if (this.logger.logsDebug()) {
                this.logger.debug("Reconnecting in " + timeSinceLastConnectAttempt + "ms");
            }
    ...
    public void auth(String credential, Firebase.AuthListener listener) {
        AuthCredential authCredential2 = this.authCredential;
        if (authCredential2 == null) {
            this.authCredential = new AuthCredential(listener, credential);
        } else if (authCredential2.matches(credential)) {
        ...
        sendAction(action, request, new ResponseListener() {
            public void onResponse(Map<String, Object> response) {
                ...
                if (completionListener != null) {
                    completionListener.onComplete(error, (Firebase) null);
                }
            }
        });
    }
    ...
    private void onDataPush(String action, Map<String, Object> body) {
        ...
        if (this.logger.logsDebug()) {
            this.logger.debug("handleServerMessage: " + str + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + map);
        }
        if (str.equals("d") || str.equals("m")) {
            boolean isMerge = str.equals("m");
            String pathString = (String) map.get("p");
            Object payloadData2 = map.get("d");
            Long tagNumber = Utilities.longFromObject(map.get("t"));
            ...
            Object payloadData3 = map.get("d");
            Long tagNumber2 = Utilities.longFromObject(map.get("t"));
            Tag tag2 = tagNumber2 != null ? new Tag(tagNumber2.longValue()) : null;
            List<Map<String, Object>> ranges = (List) payloadData3;
            ...
            for (Map<String, Object> range : ranges) {
                ...
                if (endString != null) {
                    ...
                    end = new Path(endString);
                } else {
                    payloadData = payloadData3;
                    end = null;
                }
                rangeMerges.add(new RangeMerge(start, end, NodeUtilities.NodeFromJSON(range.get("m"))));
                payloadData3 = payloadData;
                ranges = ranges;
                tagNumber2 = tagNumber2;
            }
            Long l = tagNumber2;
            ...
            this.logger.debug("Unrecognized action from server: " + str);
        }
    }
    ...
    private void sendAuthHelper(final boolean restoreWritesAfterComplete) {
        ...
        } else if (this.authCredential != null) {
            Map<String, Object> request = new HashMap<>();
            ...
            sendAction(action, put.getRequest(), new ResponseListener() {
                public void onResponse(Map<String, Object> response) {
                    if (PersistentConnection.this.logger.logsDebug()) {
                        PersistentConnection.this.logger.debug(str + " response: " + response);
                    }
    ...
    public void warnOnListenerWarnings(List<String> warnings, QuerySpec query) {
        if (warnings.contains("no_index")) {
            this.logger.warn("Using an unspecified index. Consider adding '" + ("\".indexOn\": \"" + query.getIndex().getQueryDefinition() + Typography.quote) + "' at " + query.getPath() + " to your security and Firebase rules for better performance");
        }
    }
    ...
    private void sendConnectStats() {
        ...
        } else if (!this.ctx.isPersistenceEnabled()) {
