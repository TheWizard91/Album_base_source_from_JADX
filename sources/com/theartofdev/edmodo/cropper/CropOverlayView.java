...
public class CropOverlayView extends View {
    ...
    private void drawBackground(Canvas canvas) {
        ...
        if (this.mCropShape != CropImageView.CropShape.RECTANGLE) {
            ...
            float f = left;
            float f2 = right;
            canvas2.drawRect(f, top, f2, rect.top, this.mBackgroundPaint);
            canvas2.drawRect(f, rect.bottom, f2, bottom, this.mBackgroundPaint);
    ...
    private void drawGuidelines(Canvas canvas) {
        if (this.mGuidelinePaint != null) {
            ...
            if (this.mCropShape == CropImageView.CropShape.OVAL) {
                ...
                canvas.drawLine((rect.left + w) - xv2, y2, (rect.right - w) + xv2, y2, this.mGuidelinePaint);
                return;
            }
    ...
    private boolean calculateBounds(RectF rect) {
        ...
        float b3 = y2 - (a1 * x2);
        float f = x0;
        float c0 = (rect.centerY() - rectF.top) / (rect.centerX() - rectF.left);
        float c1 = -c0;
        float f2 = y0;
        float f3 = x2;
        float d0 = rectF.top - (rectF.left * c0);
        float f4 = y2;
        float d1 = rectF.top - (rectF.right * c1);
        float left2 = Math.max(left, (d0 - b0) / (a0 - c0) < rectF.right ? (d0 - b0) / (a0 - c0) : left);
        float left3 = Math.max(left2, (d0 - b1) / (a1 - c0) < rectF.right ? (d0 - b1) / (a1 - c0) : left2);
        float left4 = Math.max(left3, (d1 - b3) / (a1 - c1) < rectF.right ? (d1 - b3) / (a1 - c1) : left3);
        float right2 = Math.min(right, (d1 - b1) / (a1 - c1) > rectF.left ? (d1 - b1) / (a1 - c1) : right);
        float right3 = Math.min(right2, (d1 - b2) / (a0 - c1) > rectF.left ? (d1 - b2) / (a0 - c1) : right2);
        float right4 = Math.min(right3, (d0 - b2) / (a0 - c0) > rectF.left ? (d0 - b2) / (a0 - c0) : right3);
