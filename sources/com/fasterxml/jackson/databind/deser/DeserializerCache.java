    ...
    private JavaType modifyTypeByAnnotation(DeserializationContext deserializationContext, Annotated annotated, JavaType javaType) throws JsonMappingException {
        ...
        if (findDeserializationKeyType != null) {
            if (javaType instanceof MapLikeType) {
                try {
                    ...
                    throw new JsonMappingException("Failed to narrow key type " + javaType + " with key-type annotation (" + findDeserializationKeyType.getName() + "): " + e2.getMessage(), (JsonLocation) null, e2);
                }
            } else {
        ...
        if (findDeserializationContentType != null) {
            try {
                ...
                throw new JsonMappingException("Failed to narrow content type " + javaType + " with content-type annotation (" + findDeserializationContentType.getName() + "): " + e3.getMessage(), (JsonLocation) null, e3);
            }
        }
        ...
        if (jsonDeserializer != null) {
            return javaType.withContentValueHandler(jsonDeserializer);
        }
        return javaType;
    }
