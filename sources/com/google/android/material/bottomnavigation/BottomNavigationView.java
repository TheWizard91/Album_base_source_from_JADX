...
public class BottomNavigationView extends FrameLayout {
    private static final int DEF_STYLE_RES = C2413R.C2419style.Widget_Design_BottomNavigationView;
    ...
    public BottomNavigationView(Context context, AttributeSet attrs) {
        this(context, attrs, C2413R.attr.bottomNavigationStyle);
    }
    ...
    public BottomNavigationView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TintTypedArray a = ThemeEnforcement.obtainTintedStyledAttributes(context2, attrs, C2413R.styleable.BottomNavigationView, defStyleAttr, C2413R.C2419style.Widget_Design_BottomNavigationView, C2413R.styleable.BottomNavigationView_itemTextAppearanceInactive, C2413R.styleable.BottomNavigationView_itemTextAppearanceActive);
        if (a.hasValue(C2413R.styleable.BottomNavigationView_itemIconTint)) {
            bottomNavigationMenuView.setIconTintList(a.getColorStateList(C2413R.styleable.BottomNavigationView_itemIconTint));
        } else {
            bottomNavigationMenuView.setIconTintList(bottomNavigationMenuView.createDefaultColorStateList(16842808));
        }
        setItemIconSize(a.getDimensionPixelSize(C2413R.styleable.BottomNavigationView_itemIconSize, getResources().getDimensionPixelSize(C2413R.dimen.design_bottom_navigation_icon_size)));
        if (a.hasValue(C2413R.styleable.BottomNavigationView_itemTextAppearanceInactive)) {
            setItemTextAppearanceInactive(a.getResourceId(C2413R.styleable.BottomNavigationView_itemTextAppearanceInactive, 0));
        }
        if (a.hasValue(C2413R.styleable.BottomNavigationView_itemTextAppearanceActive)) {
            setItemTextAppearanceActive(a.getResourceId(C2413R.styleable.BottomNavigationView_itemTextAppearanceActive, 0));
        }
        if (a.hasValue(C2413R.styleable.BottomNavigationView_itemTextColor)) {
            setItemTextColor(a.getColorStateList(C2413R.styleable.BottomNavigationView_itemTextColor));
        }
        if (getBackground() == null || (getBackground() instanceof ColorDrawable)) {
            ViewCompat.setBackground(this, createMaterialShapeDrawableBackground(context2));
        }
        if (a.hasValue(C2413R.styleable.BottomNavigationView_elevation)) {
            ViewCompat.setElevation(this, (float) a.getDimensionPixelSize(C2413R.styleable.BottomNavigationView_elevation, 0));
        }
        DrawableCompat.setTintList(getBackground().mutate(), MaterialResources.getColorStateList(context2, a, C2413R.styleable.BottomNavigationView_backgroundTint));
        setLabelVisibilityMode(a.getInteger(C2413R.styleable.BottomNavigationView_labelVisibilityMode, -1));
        setItemHorizontalTranslationEnabled(a.getBoolean(C2413R.styleable.BottomNavigationView_itemHorizontalTranslationEnabled, true));
        int itemBackground = a.getResourceId(C2413R.styleable.BottomNavigationView_itemBackground, 0);
        if (itemBackground != 0) {
            bottomNavigationMenuView.setItemBackgroundRes(itemBackground);
        } else {
            setItemRippleColor(MaterialResources.getColorStateList(context2, a, C2413R.styleable.BottomNavigationView_itemRippleColor));
        }
        if (a.hasValue(C2413R.styleable.BottomNavigationView_menu)) {
            inflateMenu(a.getResourceId(C2413R.styleable.BottomNavigationView_menu, 0));
        }
        a.recycle();
    ...
    private void addCompatibilityTopDivider(Context context) {
        ...
        divider.setBackgroundColor(ContextCompat.getColor(context, C2413R.C2414color.design_bottom_navigation_shadow_color));
        divider.setLayoutParams(new FrameLayout.LayoutParams(-1, getResources().getDimensionPixelSize(C2413R.dimen.design_bottom_navigation_shadow_height)));
        addView(divider);
    }
