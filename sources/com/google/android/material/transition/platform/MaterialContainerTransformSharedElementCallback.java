...
public class MaterialContainerTransformSharedElementCallback extends SharedElementCallback {
    ...
    public View onCreateSnapshotView(Context context, Parcelable snapshot) {
        ...
        if (!(snapshotView == null || (weakReference = capturedSharedElement) == null || this.shapeProvider == null || (sharedElement = (View) weakReference.get()) == null || (shapeAppearanceModel = this.shapeProvider.provideShape(sharedElement)) == null)) {
            snapshotView.setTag(C2413R.C2416id.mtrl_motion_snapshot_view, shapeAppearanceModel);
        }
        return snapshotView;
    }
    ...
    public void onSharedElementStart(List<String> list, List<View> sharedElements, List<View> sharedElementSnapshots) {
        if (!sharedElements.isEmpty() && !sharedElementSnapshots.isEmpty()) {
            sharedElements.get(0).setTag(C2413R.C2416id.mtrl_motion_snapshot_view, sharedElementSnapshots.get(0));
        }
        if (!this.entering && !sharedElements.isEmpty() && this.returnEndBounds != null) {
            View sharedElement = sharedElements.get(0);
            sharedElement.measure(View.MeasureSpec.makeMeasureSpec(this.returnEndBounds.width(), Ints.MAX_POWER_OF_TWO), View.MeasureSpec.makeMeasureSpec(this.returnEndBounds.height(), Ints.MAX_POWER_OF_TWO));
    ...
    public void onSharedElementEnd(List<String> list, List<View> sharedElements, List<View> list2) {
        if (!sharedElements.isEmpty() && (sharedElements.get(0).getTag(C2413R.C2416id.mtrl_motion_snapshot_view) instanceof View)) {
            sharedElements.get(0).setTag(C2413R.C2416id.mtrl_motion_snapshot_view, (Object) null);
        }
