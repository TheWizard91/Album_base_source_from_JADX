...
final class SQLiteRemoteDocumentCache implements RemoteDocumentCache {
    ...
    public void add(MaybeDocument maybeDocument, SnapshotVersion readTime) {
        ...
        this.f1716db.execute("INSERT OR REPLACE INTO remote_documents (path, read_time_seconds, read_time_nanos, contents) VALUES (?, ?, ?, ?)", path, Long.valueOf(timestamp.getSeconds()), Integer.valueOf(timestamp.getNanoseconds()), message.toByteArray());
        this.f1716db.getIndexManager().addToCollectionParentIndex((ResourcePath) maybeDocument.getKey().getPath().popLast());
    }

    public void remove(DocumentKey documentKey) {
        String path = pathForKey(documentKey);
        this.f1716db.execute("DELETE FROM remote_documents WHERE path = ?", path);
    }

    public MaybeDocument get(DocumentKey documentKey) {
        String path = pathForKey(documentKey);
        return (MaybeDocument) this.f1716db.query("SELECT contents FROM remote_documents WHERE path = ?").binding(path).firstValue(SQLiteRemoteDocumentCache$$Lambda$1.lambdaFactory$(this));
    }
    ...
    public Map<DocumentKey, MaybeDocument> getAll(Iterable<DocumentKey> documentKeys) {
        ...
        SQLitePersistence.LongQuery longQuery = new SQLitePersistence.LongQuery(this.f1716db, "SELECT contents FROM remote_documents WHERE path IN (", args, ") ORDER BY path");
    ...
    public ImmutableSortedMap<DocumentKey, Document> getAllDocumentsMatchingQuery(Query query, SnapshotVersion sinceReadTime) {
        ...
        if (sinceReadTime.equals(SnapshotVersion.NONE)) {
            sqlQuery = this.f1716db.query("SELECT path, contents FROM remote_documents WHERE path >= ? AND path < ?").binding(prefixPath, prefixSuccessorPath);
        } else {
            sqlQuery = this.f1716db.query("SELECT path, contents FROM remote_documents WHERE path >= ? AND path < ?AND (read_time_seconds > ? OR (read_time_seconds = ? AND read_time_nanos > ?))").binding(prefixPath, prefixSuccessorPath, Long.valueOf(readTime.getSeconds()), Long.valueOf(readTime.getSeconds()), Integer.valueOf(readTime.getNanoseconds()));
        }
