...
class SQLiteSchema {
    ...
    /* renamed from: db */
    private final SQLiteDatabase f1717db;
    ...
    static /* synthetic */ void lambda$createV1MutationQueue$0(SQLiteSchema sQLiteSchema) {
        sQLiteSchema.f1717db.execSQL("CREATE TABLE mutation_queues (uid TEXT PRIMARY KEY, last_acknowledged_batch_id INTEGER, last_stream_token BLOB)");
        sQLiteSchema.f1717db.execSQL("CREATE TABLE mutations (uid TEXT, batch_id INTEGER, mutations BLOB, PRIMARY KEY (uid, batch_id))");
        sQLiteSchema.f1717db.execSQL("CREATE TABLE document_mutations (uid TEXT, path TEXT, batch_id INTEGER, PRIMARY KEY (uid, path, batch_id))");
    }

    private void removeAcknowledgedMutations() {
        new SQLitePersistence.Query(this.f1717db, "SELECT uid, last_acknowledged_batch_id FROM mutation_queues").forEach(SQLiteSchema$$Lambda$2.lambdaFactory$(this));
    }
    ...
    static /* synthetic */ void lambda$removeAcknowledgedMutations$2(SQLiteSchema sQLiteSchema, Cursor mutationQueueEntry) {
        ...
        new SQLitePersistence.Query(sQLiteSchema.f1717db, "SELECT batch_id FROM mutations WHERE uid = ? AND batch_id <= ?").binding(uid, Long.valueOf(lastAcknowledgedBatchId)).forEach(SQLiteSchema$$Lambda$14.lambdaFactory$(sQLiteSchema, uid));
    }

    /* access modifiers changed from: private */
    public void removeMutationBatch(String uid, int batchId) {
        SQLiteStatement mutationDeleter = this.f1717db.compileStatement("DELETE FROM mutations WHERE uid = ? AND batch_id = ?");
        ...
        this.f1717db.execSQL("DELETE FROM document_mutations WHERE uid = ? AND batch_id = ?", new Object[]{uid, Integer.valueOf(batchId)});
    }
    ...
    static /* synthetic */ void lambda$createV1TargetCache$3(SQLiteSchema sQLiteSchema) {
        sQLiteSchema.f1717db.execSQL("CREATE TABLE targets (target_id INTEGER PRIMARY KEY, canonical_id TEXT, snapshot_version_seconds INTEGER, snapshot_version_nanos INTEGER, resume_token BLOB, last_listen_sequence_number INTEGER,target_proto BLOB)");
        sQLiteSchema.f1717db.execSQL("CREATE INDEX query_targets ON targets (canonical_id, target_id)");
        sQLiteSchema.f1717db.execSQL("CREATE TABLE target_globals (highest_target_id INTEGER, highest_listen_sequence_number INTEGER, last_remote_snapshot_version_seconds INTEGER, last_remote_snapshot_version_nanos INTEGER)");
        sQLiteSchema.f1717db.execSQL("CREATE TABLE target_documents (target_id INTEGER, path TEXT, PRIMARY KEY (target_id, path))");
        sQLiteSchema.f1717db.execSQL("CREATE INDEX document_targets ON target_documents (path, target_id)");
    }

    private void dropV1TargetCache() {
        if (tableExists("targets")) {
            this.f1717db.execSQL("DROP TABLE targets");
        }
        if (tableExists("target_globals")) {
            this.f1717db.execSQL("DROP TABLE target_globals");
        }
        if (tableExists("target_documents")) {
            this.f1717db.execSQL("DROP TABLE target_documents");
        }
    }
    ...
    private void ensureTargetGlobal() {
        if (!(DatabaseUtils.queryNumEntries(this.f1717db, "target_globals") == 1)) {
            this.f1717db.execSQL("INSERT INTO target_globals (highest_target_id, highest_listen_sequence_number, last_remote_snapshot_version_seconds, last_remote_snapshot_version_nanos) VALUES (?, ?, ?, ?)", new String[]{"0", "0", "0", "0"});
        }
    }

    private void addTargetCount() {
        if (!tableContainsColumn("target_globals", "target_count")) {
            this.f1717db.execSQL("ALTER TABLE target_globals ADD COLUMN target_count INTEGER");
        }
        long count = DatabaseUtils.queryNumEntries(this.f1717db, "targets");
        ContentValues cv = new ContentValues();
        cv.put("target_count", Long.valueOf(count));
        this.f1717db.update("target_globals", cv, (String) null, (String[]) null);
    }

    private void addSequenceNumber() {
        if (!tableContainsColumn("target_documents", "sequence_number")) {
            this.f1717db.execSQL("ALTER TABLE target_documents ADD COLUMN sequence_number INTEGER");
        }
    }
    ...
    private void addReadTime() {
        this.f1717db.execSQL("ALTER TABLE remote_documents ADD COLUMN read_time_seconds INTEGER");
        this.f1717db.execSQL("ALTER TABLE remote_documents ADD COLUMN read_time_nanos INTEGER");
    }

    private void dropLastLimboFreeSnapshotVersion() {
        new SQLitePersistence.Query(this.f1717db, "SELECT target_id, target_proto FROM targets").forEach(SQLiteSchema$$Lambda$6.lambdaFactory$(this));
    }
    ...
    static /* synthetic */ void lambda$dropLastLimboFreeSnapshotVersion$6(SQLiteSchema sQLiteSchema, Cursor cursor) {
        ...
        try {
            ...
            sQLiteSchema.f1717db.execSQL("UPDATE targets SET target_proto = ? WHERE target_id = ?", new Object[]{targetProto.toByteArray(), Integer.valueOf(targetId)});
    ...
    private void ensureSequenceNumbers() {
        Long boxedSequenceNumber = (Long) new SQLitePersistence.Query(this.f1717db, "SELECT highest_listen_sequence_number FROM target_globals LIMIT 1").firstValue(SQLiteSchema$$Lambda$7.lambdaFactory$());
        ...
        SQLiteStatement tagDocument = this.f1717db.compileStatement("INSERT INTO target_documents (target_id, path, sequence_number) VALUES (0, ?, ?)");
        SQLitePersistence.Query untaggedDocumentsQuery = new SQLitePersistence.Query(this.f1717db, "SELECT RD.path FROM remote_documents AS RD WHERE NOT EXISTS (SELECT TD.path FROM target_documents AS TD WHERE RD.path = TD.path AND TD.target_id = 0) LIMIT ?").binding(100);
    ...
    private void createV8CollectionParentsIndex() {
        ...
        Consumer<ResourcePath> addEntry = SQLiteSchema$$Lambda$10.lambdaFactory$(new MemoryIndexManager.MemoryCollectionParentIndex(), this.f1717db.compileStatement("INSERT OR REPLACE INTO collection_parents (collection_id, parent) VALUES (?, ?)"));
        new SQLitePersistence.Query(this.f1717db, "SELECT path FROM remote_documents").forEach(SQLiteSchema$$Lambda$11.lambdaFactory$(addEntry));
        new SQLitePersistence.Query(this.f1717db, "SELECT path FROM document_mutations").forEach(SQLiteSchema$$Lambda$12.lambdaFactory$(addEntry));
    }
    ...
    public List<String> getTableColumns(String table) {
        ...
        try {
            c = this.f1717db.rawQuery("PRAGMA table_info(" + table + ")", (String[]) null);
    ...
    private void rewriteCanonicalIds() {
        new SQLitePersistence.Query(this.f1717db, "SELECT target_id, target_proto FROM targets").forEach(SQLiteSchema$$Lambda$13.lambdaFactory$(this));
    }
    ...
    static /* synthetic */ void lambda$rewriteCanonicalIds$13(SQLiteSchema sQLiteSchema, Cursor cursor) {
        ...
        try {
            ...
            sQLiteSchema.f1717db.execSQL("UPDATE targets SET canonical_id  = ? WHERE target_id = ?", new Object[]{updatedCanonicalId, Integer.valueOf(targetId)});
        } catch (InvalidProtocolBufferException e) {
            throw Assert.fail("Failed to decode Query data for target %s", Integer.valueOf(targetId));
        }
    }
    ...
    private boolean tableExists(String table) {
        return !new SQLitePersistence.Query(this.f1717db, "SELECT 1=1 FROM sqlite_master WHERE tbl_name = ?").binding(table).isEmpty();
    }
}
