...
final class SQLiteTargetCache implements TargetCache {
    ...
    /* renamed from: db */
    private final SQLitePersistence f1718db;
    ...
    public void start() {
        boolean z = true;
        if (this.f1718db.query("SELECT highest_target_id, highest_listen_sequence_number, last_remote_snapshot_version_seconds, last_remote_snapshot_version_nanos, target_count FROM target_globals LIMIT 1").first(SQLiteTargetCache$$Lambda$1.lambdaFactory$(this)) != 1) {
            z = false;
        }
    ...
    public void forEachTarget(Consumer<TargetData> consumer) {
        this.f1718db.query("SELECT target_proto FROM targets").forEach(SQLiteTargetCache$$Lambda$2.lambdaFactory$(this, consumer));
    }
    ...
    public int removeQueries(long upperBound, SparseArray<?> activeTargetIds) {
        int[] count = new int[1];
        this.f1718db.query("SELECT target_id FROM targets WHERE last_listen_sequence_number <= ?").binding(Long.valueOf(upperBound)).forEach(SQLiteTargetCache$$Lambda$3.lambdaFactory$(this, activeTargetIds, count));
        writeMetadata();
        return count[0];
    }
    ...
    public TargetData getTargetData(com.google.firebase.firestore.core.Target target) {
        ...
        this.f1718db.query("SELECT target_proto FROM targets WHERE canonical_id = ?").binding(canonicalId).forEach(SQLiteTargetCache$$Lambda$4.lambdaFactory$(this, target, result));
    ...
    public void addMatchingKeys(ImmutableSortedSet<DocumentKey> keys, int targetId) {
        SQLiteStatement inserter = this.f1718db.prepare("INSERT OR IGNORE INTO target_documents (target_id, path) VALUES (?, ?)");
        ReferenceDelegate delegate = this.f1718db.getReferenceDelegate();
        Iterator<DocumentKey> it = keys.iterator();
        while (it.hasNext()) {
            DocumentKey key = it.next();
            String path = EncodedPath.encode(key.getPath());
            this.f1718db.execute(inserter, Integer.valueOf(targetId), path);
            delegate.addReference(key);
        }
    }

    public void removeMatchingKeys(ImmutableSortedSet<DocumentKey> keys, int targetId) {
        SQLiteStatement deleter = this.f1718db.prepare("DELETE FROM target_documents WHERE target_id = ? AND path = ?");
        ReferenceDelegate delegate = this.f1718db.getReferenceDelegate();
        Iterator<DocumentKey> it = keys.iterator();
        while (it.hasNext()) {
            DocumentKey key = it.next();
            String path = EncodedPath.encode(key.getPath());
            this.f1718db.execute(deleter, Integer.valueOf(targetId), path);
            delegate.removeReference(key);
        }
    }

    private void removeMatchingKeysForTargetId(int targetId) {
        this.f1718db.execute("DELETE FROM target_documents WHERE target_id = ?", Integer.valueOf(targetId));
    }
    ...
    public ImmutableSortedSet<DocumentKey> getMatchingKeysForTargetId(int targetId) {
        ...
        this.f1718db.query("SELECT path FROM target_documents WHERE target_id = ?").binding(Integer.valueOf(targetId)).forEach(SQLiteTargetCache$$Lambda$5.lambdaFactory$(holder));
        return holder.keys;
    }
    ...
    public boolean containsKey(DocumentKey key) {
        String path = EncodedPath.encode(key.getPath());
        return !this.f1718db.query("SELECT target_id FROM target_documents WHERE path = ? AND target_id != 0 LIMIT 1").binding(path).isEmpty();
    }
}
