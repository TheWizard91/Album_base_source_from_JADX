...
final class SQLiteIndexManager implements IndexManager {
    ...
    /* renamed from: db */
    private final SQLitePersistence f1711db;

    SQLiteIndexManager(SQLitePersistence persistence) {
        this.f1711db = persistence;
    }
    ...
    public void addToCollectionParentIndex(ResourcePath collectionPath) {
        ...
        if (this.collectionParentsCache.add(collectionPath)) {
            this.f1711db.execute("INSERT OR REPLACE INTO collection_parents (collection_id, parent) VALUES (?, ?)", collectionPath.getLastSegment(), EncodedPath.encode((ResourcePath) collectionPath.popLast()));
        }
    }

    public List<ResourcePath> getCollectionParents(String collectionId) {
        ArrayList<ResourcePath> parentPaths = new ArrayList<>();
        this.f1711db.query("SELECT parent FROM collection_parents WHERE collection_id = ?").binding(collectionId).forEach(SQLiteIndexManager$$Lambda$1.lambdaFactory$(parentPaths));
        return parentPaths;
    }
}
