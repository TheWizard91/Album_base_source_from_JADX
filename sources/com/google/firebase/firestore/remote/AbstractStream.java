...
abstract class AbstractStream<ReqT, RespT, CallbackT extends Stream.StreamCallback> implements Stream<CallbackT> {
            ...
            if (Logger.isDebugEnabled()) {
                ...
                for (String header : headers.keys()) {
                    if (Datastore.WHITE_LISTED_HEADERS.contains(header.toLowerCase(Locale.ENGLISH))) {
                        whitelistedHeaders.put(header, (String) headers.get(Metadata.Key.m188of(header, Metadata.ASCII_STRING_MARSHALLER)));
                    }
                }
    ...
    private void close(Stream.State finalState, Status status) {
        ...
        Assert.hardAssert(finalState == Stream.State.Error || status.equals(Status.f193OK), "Can't provide an error when not in an error state.", new Object[0]);
        this.workerQueue.verifyIsCurrentThread();
    ...
    public void stop() {
        if (isStarted()) {
            close(Stream.State.Initial, Status.f193OK);
        }
    }
