...
public class ChildChangeAccumulator {
    ...
    public void trackChildChange(Change change) {
        ...
        if (type != Event.EventType.CHILD_ADDED && type != Event.EventType.CHILD_CHANGED && type != Event.EventType.CHILD_REMOVED) {
            throw new AssertionError("Only child changes supported for tracking");
        } else if (change.getChildKey().isPriorityChildName()) {
