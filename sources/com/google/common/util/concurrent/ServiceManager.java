...
public final class ServiceManager {
    ...
    public ServiceManager(Iterable<? extends Service> services2) {
        ...
        if (copy.isEmpty()) {
            ...
            copy = ImmutableList.m1138of(new NoOpService());
        }
    ...
    private static final class ServiceManagerState {
        ...
        public void awaitHealthy(long timeout, TimeUnit unit) throws TimeoutException {
            ...
            try {
                ...
                throw new TimeoutException("Timeout waiting for the services to become healthy. The following services have not started: " + Multimaps.filterKeys(this.servicesByState, Predicates.m1112in(ImmutableSet.m1181of(Service.State.NEW, Service.State.STARTING))));
            } finally {
                this.monitor.leave();
            }
        }
        ...
        public void awaitStopped(long timeout, TimeUnit unit) throws TimeoutException {
            ...
            try {
                if (!this.monitor.waitForUninterruptibly(this.stoppedGuard, timeout, unit)) {
                    throw new TimeoutException("Timeout waiting for the services to stop. The following services have not stopped: " + Multimaps.filterKeys(this.servicesByState, Predicates.not(Predicates.m1112in(EnumSet.of(Service.State.TERMINATED, Service.State.FAILED)))));
                }
            } finally {
                this.monitor.leave();
            }
