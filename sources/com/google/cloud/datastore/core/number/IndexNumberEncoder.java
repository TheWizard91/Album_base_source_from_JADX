...
public class IndexNumberEncoder {
    ...
    public static int encodeDouble(boolean descending, double value, byte[] buffer, int offset) {
        ...
        int exponent2 = ((int) ((doubleBits >>> 52) & 2047)) - 1023;
    ...
    private static int encodeNumber(boolean invertEncoding, int exponent, long significand, byte[] buffer, int offset) {
        ...
        if (exponent < 4) {
            ...
            int exponent2 = exponent - 4;
            if (exponent2 <= 15) {
                buffer[lastByte2] = (byte) ((224 | exponent2) ^ ((exponentMask & WorkQueueKt.MASK) ^ inverter));
            ...
            int exponent3 = exponent - 20;
            if (exponent3 <= 127) {
                int bufferPos = lastByte2 + 1;
                buffer[lastByte2] = (byte) ((240 | (exponent3 >>> 4)) ^ ((exponentMask & WorkQueueKt.MASK) ^ inverter));
                long significand3 = significand << 4;
                buffer[bufferPos] = (byte) ((((exponent3 << 4) & 240) | ((int) (significand >>> 60))) ^ ((exponentMask & 240) ^ inverter));
            ...
            int exponent4 = exponent - 148;
            if (exponent4 <= 1023) {
                int bufferPos2 = lastByte2 + 1;
                buffer[lastByte2] = (byte) ((248 | (exponent4 >>> 8)) ^ ((exponentMask & WorkQueueKt.MASK) ^ inverter));
                buffer[bufferPos2] = (byte) ((exponent4 & 255) ^ ((exponentMask & 255) ^ inverter));
