...
public final class FlowableBuffer<T, C extends Collection<? super T>> extends AbstractFlowableWithUpstream<T, C> {
    ...
    static final class PublisherBufferExactSubscriber<T, C extends Collection<? super T>> implements FlowableSubscriber<T>, Subscription {
        ...
        public void request(long n) {
            if (SubscriptionHelper.validate(n)) {
                this.f256s.request(BackpressureHelper.multiplyCap(n, (long) this.size));
            }
        }
        ...
        public void onSubscribe(Subscription s) {
            if (SubscriptionHelper.validate(this.f256s, s)) {
                this.f256s = s;
    ...
    static final class PublisherBufferSkipSubscriber<T, C extends Collection<? super T>> extends AtomicInteger implements FlowableSubscriber<T>, Subscription {
        ...
        public void request(long n) {
            ...
            if (get() != 0 || !compareAndSet(0, 1)) {
                this.f258s.request(BackpressureHelper.multiplyCap((long) this.skip, n));
                return;
            }
            this.f258s.request(BackpressureHelper.addCap(BackpressureHelper.multiplyCap(n, (long) this.size), BackpressureHelper.multiplyCap((long) (this.skip - this.size), n - 1)));
        }
        ...
        public void onSubscribe(Subscription s) {
            if (SubscriptionHelper.validate(this.f258s, s)) {
                this.f258s = s;
                this.actual.onSubscribe(this);
            }
        }
    ...
    static final class PublisherBufferOverlappingSubscriber<T, C extends Collection<? super T>> extends AtomicLong implements FlowableSubscriber<T>, Subscription, BooleanSupplier {
            ...
            if (SubscriptionHelper.validate(n)) {
                if (!QueueDrainHelper.postCompleteRequest(n, this.actual, this.buffers, this, this)) {
                    if (this.once.get() || !this.once.compareAndSet(false, true)) {
                        this.f257s.request(BackpressureHelper.multiplyCap((long) this.skip, n));
                        return;
                    }
                    this.f257s.request(BackpressureHelper.addCap((long) this.size, BackpressureHelper.multiplyCap((long) this.skip, n - 1)));
                }
            }
        }
        ...
        public void onSubscribe(Subscription s) {
            if (SubscriptionHelper.validate(this.f257s, s)) {
                this.f257s = s;
                this.actual.onSubscribe(this);
            }
        }
